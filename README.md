...
簡報連結：[點此查看完整簡報](https://canva.link/yhwnoiy4a9hppb5) 📊
...
* **範圍鎖定**：過濾掉非 WA (華盛頓州) 的資料。
* **缺失值處理**：
    * 刪除關鍵欄位（`County`, `City`, `Postal Code`, `Electric Range`）含有缺失值的紀錄。
    * 將 `Legislative District` 缺失值補上 `Unknown`。
* **型態轉換**：將 `Postal Code` 與 `Legislative District` 轉換為字串類型。

---

## 主要分析洞察 💡
1.  **市場佔比**：Tesla 在華盛頓州具有絕對的品牌支配力，佔比高達 40.86%。
2.  **區域熱點**：King County 為全州電動車數量最多之區域，建議優先在此進行基礎設施佈建。
3.  **品牌與型號**：除了 Tesla 穩居龍頭外，CHEVROLET 與 NISSAN 亦表現亮眼，而 Model Y 為熱門型號首選。
